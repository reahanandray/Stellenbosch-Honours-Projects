...
print("Leaf Dataset using XGBoost:")
print(".")
...
input_data = rawASD.T
input_data.columns = rawASD.iloc[:, 0]
...
# ===============================================================================
#             First iteration - Run data as it is with XGBoost
# ===============================================================================
...
X = input_data.drop(columns=['Cultivar'])
y = input_data['Cultivar']
...
# 2. TRAIN AND TEST ON TRAINING AND TEST DATA - XGBoost Model
...
# 3. PRODUCE ACCURACY MATRIX FOR TRAIN AND TEST DATA
...
# Print results
print(f"\nXGBoost Training Set Accuracy: {train_accuracy:.2f}")
print("\nXGBoost Training Classification Report:\n", train_classification_rep)

print(f"\nXGBoost Test Set Accuracy: {test_accuracy:.2f}")
...
print(f"XGBoost Training MCC: {mcc_train:.2f}")
print(f"XGBoost Test MCC: {mcc_test:.2f}")
...
print('XGBoost ROC AUC Test Score :', roc_auc)

# ROC curve for Multi classes
colors = ['orange', 'red', 'green']
class_names = label_encoder.classes_
for i in range(len(class_names)):
    fpr, tpr, thresh = roc_curve(y_test, y_pred_prob[:, i], pos_label=i)  # FIXED: Use y_pred_prob
...
plt.title('XGBoost Grapevine Cultivar ROC Curve on Test Set')
...
# ===============================================================================
#             Second iteration - Feature Selection using Boruta with XGBoost
# ===============================================================================
...
# Initialize Boruta feature selection with XGBClassifier
...
# 2. TRAIN AND TEST ON SUBSET TRAINING AND TEST DATA - XGB Model

# Train XGBoost on the selected features
xgb_classifier_selected = XGBClassifier(n_estimators=100, random_state=42)
...
# Calculate accuracy and classification report for train and test
...
# Print the results
print(f"\nXGBoost with Boruta Feature Selection - Training Accuracy: {train_accuracy_selected:.2f}")
print("\nXGBoost Train Classification Report (After Feature Selection):\n", train_classification_rep_selected)

print(f"XGBoost with Boruta Feature Selection - Test Accuracy: {test_accuracy_selected:.2f}")
print("\nXGBoost Test Classification Report (After Feature Selection):\n", test_classification_rep_selected)

# Calculate MCC for train and test sets
mcc_train = matthews_corrcoef(y_train, y_train_pred_selected)
mcc_test = matthews_corrcoef(y_test, y_test_pred_selected)
...
# Compute the ROC AUC score
y_pred_prob_selected = xgb_classifier_selected.predict_proba(X_test_selected)
roc_auc_selected = roc_auc_score(y_test, y_pred_prob_selected, multi_class='ovr')
print('XGBoost ROC AUC Test Score (After Feature Selection):', roc_auc_selected)
...
plt.plot([0, 1], [0, 1], 'k--', label='Random classifier')
plt.title('XGBoost ROC Curve on Test Set (After Feature Selection)')
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.legend()
plt.show()
...
# ===============================================================================
#          Third iteration - Account for Class Imbalance on a Data-Level with XGBoost
# ===============================================================================
...
# Train XGBoost on the resampled training data
xgb_classifier_resampled = XGBClassifier(n_estimators=100, random_state=42)
xgb_classifier_resampled.fit(X_train_resampled, y_train_resampled)

# 3. PRODUCE ACCURACY MATRIX AND MCC FOR TRAIN AND TEST DATA

# Make predictions
y_train_pred_resampled = xgb_classifier_resampled.predict(X_train_resampled)
y_test_pred_resampled = xgb_classifier_resampled.predict(X_test_selected)
...
# Print the results
print(f"\nXGBoost Training Set Accuracy (After SMOTE): {train_accuracy_resampled:.2f}")
print("\nXGBoost Training Classification Report (After SMOTE):\n", train_classification_rep_resampled)
print(f"\nXGBoost Test Set Accuracy (After SMOTE): {test_accuracy_resampled:.2f}")
print("\nXGBoost Test Classification Report (After SMOTE):\n", test_classification_rep_resampled)
'''
# Calculate MCC for train and test sets
mcc_train = matthews_corrcoef(y_train, y_train_pred_resampled)
mcc_test = matthews_corrcoef(y_test, y_test_pred_resampled)
...
# Compute the ROC AUC score
y_pred_prob_resampled = xgb_classifier_resampled.predict_proba(X_test_selected)
roc_auc_resampled = roc_auc_score(y_test, y_pred_prob_resampled, multi_class='ovr')
print('XGBoost ROC AUC Test Score (After SMOTE):', roc_auc_resampled)
...
plt.plot([0, 1], [0, 1], 'k--', label='Random classifier')
plt.title('XGBoost ROC Curve on Test Set (After SMOTE)')
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.legend()
plt.show()
